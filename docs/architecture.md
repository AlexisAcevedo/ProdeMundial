...
- **Optimización de Reconciliación en React**: Uso estratégico de la propiedad `key` en componentes raíz renderizados condicionalmente (ej. estados de carga vs. contenido) para evitar el reciclaje erróneo de nodos del DOM, previniendo *glitches* y morfismos no deseados en componentes que utilizan clases de transición CSS (`transition-all`).
